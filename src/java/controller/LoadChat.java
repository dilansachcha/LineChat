...
public class LoadChat extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        ///LoadChat?logged_user_id=2&other_user_id=1
        Gson gson = new Gson();

        Session session = HibernateUtil.getSessionFactory().openSession();;

        String logged_user_id = request.getParameter("logged_user_id");//ewna data gnnw ui ptten
        ...
        Criteria criteria1 = session.createCriteria(Chat.class);//from - to users la denna arn inna oone *

        criteria1.add(
                Restrictions.or(//yta awstha 2n ekk
        ...
        //create chat array 
        JsonArray chatArray = new JsonArray();//ehapttata array ekkin ywnne
        ...
        for (Chat chat : chat_List) {
            ...
            chatObject.addProperty("message", chat.getMessage());//adala ewa witrk arn ynwa chatObj eken
            chatObject.addProperty("datetime", dateFormat.format(chat.getDate_time()));

            //get chats only from other user
            if (chat.getFrom_user().getId() == other_user.getId()) {//msg ewpu kena other kena wena

                //add side to chat object - for other user
                ...
                //add side to chat object - for logged user
                chatObject.addProperty("side", "right");
                chatObject.addProperty("status", chat.getChat_Status().getId());//1=seen , 2=unseen - spi ywpu eke status ek wdgath*
            }
